...
@router.get("/chapters/{book_id}/{chapter}/status", response_model=ChapterStatusResponse)
def get_chapter_status(
    ...
    return service.get_chapter_status(user_id, book_id, chapter)
...
def get_verses_for_practice(
    limit: int = 10,
    user_id: int = Depends(get_current_user_id),
    service: VerseService = Depends(get_verse_service)
):
    """Get verses due for practice based on spaced repetition"""
    return service.get_verses_for_practice(user_id, limit)


@router.post("/texts", response_model=List[VerseTextResponse])
def get_verse_texts(
    ...
    return service.get_verse_texts(request, bible_id)
