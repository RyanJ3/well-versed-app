...
export class BibleTrackerService {
    ...
    public calculateBookStats(bookName: string): BookStats {
        ...
        return new BookStats(
            book.getTotalVerses() ? Math.round((memorizedVerses / book.getTotalVerses()) * 100) : 0,
            memorizedVerses,
            book.getTotalVerses(),
            completedChapters,
            inProgressChapters
    ...
    public getBooksInGroup(group: string): { [key: string]: BibleBook } {
        const books = BIBLE_DATA.getBooksByGroup(group);
        const result: { [key: string]: BibleBook } = {};
    ...
    public calculateGroupStats(group: string): GroupStats {
        ...
        return new GroupStats(
            groupTotalVerses ? Math.round((groupMemorizedVerses / groupTotalVerses) * 100) : 0,
            groupCompletedChapters,
            groupTotalChapters
        );
    }
    ...
    public calculateTestamentStats(testament: string): TestamentStats {
        ...
        const booksInTestament: BibleBook[] = BIBLE_DATA.getBooksByTestament(testament);
        ...
        booksInTestament.forEach((book: BibleBook) => {
            ...
            if (bookProgress) {
                bookProgress.forEach((chapterProgress: ChapterProgress | null) => {
                    if (chapterProgress) {
                        // Count memorized verses using the versesMemorized array
                        if (chapterProgress.versesMemorized && Array.isArray(chapterProgress.versesMemorized)) {
                            memorizedVerses += chapterProgress.versesMemorized.filter((v: boolean) => v).length;
                        } else {
        ...
        const percentComplete: number = totalVerses > 0
            ? Math.round((memorizedVerses / totalVerses) * 100)
            : 0;
    ...
    public updateChapterProgress(bookName: string, chapterIndex: number, newValue: number): void {
        ...
        // Ensure the book entry exists
        if (!currentProgress[bookName]) {
            currentProgress[bookName] = Array(book.getTotalChapters()).fill(null).map((_, i) => {
    ...
    public resetGroup(group: string): void {
        ...
        booksInGroup.forEach(book => {
            currentProgress[book.name] = Array(book.getTotalChapters()).fill(null).map((_, i) => {
                return new ChapterProgress(
                    i + 1,
                    book.getVerseCount(i),
                    0,
    ...
    private fetchProgressFromAPI(): Observable<BookProgress> {
        ...
        if (psalmsBook && dummyProgress['Psalms'] && dummyProgress['Psalms'].length > 0) {
            // A few completed psalms
            for (let i = 0; i < 10; i++) {
                if (dummyProgress['Psalms'][i]) {
                    const verseCount = psalmsBook.getVerseCount(i);
                    const versesMemorized = Array(verseCount > 6 ? verseCount : 6).fill(true);
        ...
        if (romansBook && dummyProgress['Romans'] && dummyProgress['Romans'].length > 0) {
            // Romans 8 completed
    ...
    private initializeEmptyProgress(): BookProgress {
        ...
        BIBLE_DATA.books.forEach(book => {
            ...
            initialProgress[bookName] = Array(book.getTotalChapters()).fill(null).map((_, i) => {
    ...
    private migrateProgressData(progress: BookProgress): BookProgress {
        ...
        Object.entries(updatedProgress).forEach(([bookName, chapters]) => {
            const book = BIBLE_DATA.getBookByName(bookName);
            if (chapters && book) {
                updatedProgress[bookName] = chapters.map((chapter: ChapterProgress | null, chapterIndex: number) => {
                    if (!chapter) {
                        return new ChapterProgress(
                            chapterIndex + 1,
     ...
     */
    public getChapterProgress(bookName: string, chapterIndex: number): ChapterProgress | null {
        const currentProgress = this.progressSubject.value;
        if (!currentProgress[bookName] || !currentProgress[bookName][chapterIndex]) {
            return null;
        }
