...
export class BibleService {
  ...
  getUserVerses(userId: number, includeApocrypha?: boolean): Observable<UserVerseDetail[]> {
    this.normalizeUserId(userId); // retain normalization for logging if needed
  ...
  saveChapter(userId: number, bookId: number, chapterNum: number): Observable<any> {
    ...
    return this.http.post(`${this.apiUrl}/verses/chapters`, { book_id: bookId, chapter: chapterNum }).pipe(
  ...
  saveBook(userId: number, bookId: number): Observable<any> {
    ...
    return this.http.post(`${this.apiUrl}/verses/books`, { book_id: bookId }).pipe(
