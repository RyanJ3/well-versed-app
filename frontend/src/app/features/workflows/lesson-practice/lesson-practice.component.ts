    ...
    <div class="lesson-practice-container">
      ...
      <div class="settings-overlay" *ngIf="showSettings">
        <div class="settings-card">
          ...
          <button class="start-button" (click)="startPractice()" [disabled]="!versesPerSet">
            Start Practice
          </button>
        </div>
      </div>
      ...
      <div class="practice-area" *ngIf="!showSettings && currentSet">
        ...
        <div class="progress-header">
          <div class="progress-bar-container">
            ...
            <span class="progress-text">{{ currentSetIndex + 1 }} of {{ verseSets.length }}</span>
          </div>
        </div>

        <!-- Stage Indicator -->
        <div class="stage-progress">
          <div class="stage-item" [class.active]="currentSet.currentStage === 'full'" [class.completed]="stageCompleted.full">
            <div class="stage-dot"></div>
            <span>Read</span>
          </div>
          <div class="stage-line" [class.filled]="stageCompleted.full"></div>
          
          <div class="stage-item" [class.active]="currentSet.currentStage === 'initials'" [class.completed]="stageCompleted.initials">
            <div class="stage-dot"></div>
            <span>FLOW</span>
          </div>
          <div class="stage-line" [class.filled]="stageCompleted.initials"></div>
          
          <div class="stage-item" [class.active]="currentSet.currentStage === 'memory'">
            <div class="stage-dot"></div>
            <span>Recite</span>
          </div>
        </div>
        ...
        <div class="verse-container">
          ...
          <div class="verse-content">
            ...
            <!-- Initials Stage -->
            <div class="initials-text" *ngIf="currentSet.currentStage === 'initials'">
            ...
            <!-- Memory Stage -->
            <div class="memory-stage" *ngIf="currentSet.currentStage === 'memory'">
        ...
        <div class="recording-section">
          <button 
            ...
            [disabled]="!canRecord"
          >
            <div class="record-icon" *ngIf="!currentSet.audioUrl || isRecording">
            ...
            <span *ngIf="!currentSet.audioUrl">{{ isRecording ? 'Recording...' : 'Hold to Record' }}</span>
            <span *ngIf="currentSet.audioUrl && !isRecording">Recorded ✓</span>
          </button>

          <div class="audio-controls" *ngIf="currentSet.audioUrl && !isRecording">
        ...
        <div class="nav-buttons">
          <button 
            ...
            (click)="previousStage()"
            [disabled]="currentSet.currentStage === 'full' && currentSetIndex === 0"
          >
            Back
          </button>
      ...
      <div class="confidence-modal" *ngIf="showConfidence && currentSet">
        <div class="confidence-card">
          ...
          <div class="confidence-slider-container">
            ...
            >
            <div class="confidence-percentage">{{ currentSet.confidence }}%</div>
          </div>
          ...
          <button class="confidence-continue" (click)="saveConfidence()">
            {{ currentSetIndex < verseSets.length - 1 ? 'Continue' : 'Complete Lesson' }}
          </button>
        </div>
      </div>
    </div>
...
export class LessonPracticeComponent implements OnInit, OnDestroy {
  ...
  async createVerseSets() {
    const texts = await this.bibleService.getVerseTexts(this.userId, this.allVerseCodes).toPromise();
    
    for (let i = 0; i < this.allVerseCodes.length; i += this.versesPerSet) {
      const setVerses = [];
      
      for (let j = 0; j < this.versesPerSet && i + j < this.allVerseCodes.length; j++) {
  ...
  updateProgress() {
    ...
    const completedStages = this.currentSetIndex * 3 + 
