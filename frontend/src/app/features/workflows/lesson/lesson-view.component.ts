...
import { Lesson, LessonFlashcard, UserLessonProgress } from '../../../core/models/workflow.model';
...
@Component({
    ...
    <div class="lesson-view" *ngIf="lesson">
      ...
      <div class="lesson-content-section" [ngSwitch]="lesson.content_type">
        ...
        <div *ngSwitchCase="'article'" class="article-content">
          <div class="article-text" [innerHTML]="formatArticleText(lesson.content_data.article_text!)"></div>
        </div>
          ...
          <div class="flashcard-grid">
            ...
            >
              ...
              <div class="card-checkbox">
                <svg *ngIf="selectedFlashcards.includes(card.id)" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
  ...
  styles: [`
    ...
    .verse-picker-section, .premade-flashcards {
      margin-bottom: 2rem;
    }

    .verse-picker-section h3, .premade-flashcards h3 {
      font-size: 1.25rem;
      font-weight: 600;
      color: #1f2937;
      margin-bottom: 1rem;
...
export class LessonViewComponent implements OnInit {
  ...
  updateProgress() {
    const totalSelected = this.selectedFlashcards.length + this.selectedVerses.length;
    const completed = this.userProgress?.flashcards_completed || 0;
    this.progressPercentage = Math.min(((completed + totalSelected) / this.requiredFlashcards) * 100, 100);
  }
  ...
  addFlashcardsToQueue() {
    ...
    this.workflowService.addFlashcardsToQueue(request, this.userId).subscribe({
      next: () => {
        ...
        this.workflowService.completeLesson(this.lessonId, this.userId).subscribe({
          next: () => {
