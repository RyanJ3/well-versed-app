    ...
    <div class="quiz-practice-container">
      ...
      <div class="quiz-header" *ngIf="!showResults">
        ...
        <div class="quiz-progress">
          ...
          <span class="progress-text">{{ currentIndex + 1 }} of {{ quizVerses.length }}</span>
        </div>
      </div>
      ...
      <div class="practice-area" *ngIf="currentVerse && !showResults">
        ...
        <div class="stage-progress">
          <div class="stage-item" [class.active]="currentVerse.currentStage === 'full'" [class.completed]="stageCompleted.full">
            <div class="stage-dot"></div>
            <span>Read</span>
          </div>
          <div class="stage-line" [class.filled]="stageCompleted.full"></div>
          
          <div class="stage-item" [class.active]="currentVerse.currentStage === 'initials'" [class.completed]="stageCompleted.initials">
            <div class="stage-dot"></div>
            <span>FLOW</span>
          </div>
          <div class="stage-line" [class.filled]="stageCompleted.initials"></div>
          
          <div class="stage-item" [class.active]="currentVerse.currentStage === 'memory'">
            <div class="stage-dot"></div>
            <span>Recite</span>
          </div>
        </div>
        ...
        <div class="verse-container">
          ...
          <div class="verse-content">
            ...
            <!-- Initials -->
            <div class="initials-text" *ngIf="currentVerse.currentStage === 'initials'">
              <p class="verse-initials">{{ getInitialsText(currentVerse.text) }}</p>
            </div>
            
            <!-- Memory -->
            <div class="memory-stage" *ngIf="currentVerse.currentStage === 'memory'">
        ...
        <div class="recording-section">
          <button 
            ...
            [disabled]="!canRecord"
          >
            <div class="record-icon" *ngIf="!currentVerse.audioUrl || isRecording">
              <svg *ngIf="!isRecording" width="32" height="32" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/>
                <path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/>
              </svg>
              <div class="recording-pulse" *ngIf="isRecording"></div>
            </div>
            <span *ngIf="!currentVerse.audioUrl">{{ isRecording ? 'Recording...' : 'Hold to Record' }}</span>
            <span *ngIf="currentVerse.audioUrl && !isRecording">Recorded ✓</span>
          </button>

          <div class="audio-controls" *ngIf="currentVerse.audioUrl && !isRecording">
        ...
        <div class="nav-buttons">
          <button 
            ...
            (click)="previousStage()"
            [disabled]="currentVerse.currentStage === 'full' && currentIndex === 0"
          >
            Back
          </button>
      ...
      <div class="confidence-modal" *ngIf="showConfidence && currentVerse">
        <div class="confidence-card">
          ...
          <div class="confidence-slider-container">
            ...
            >
            <div class="confidence-percentage">{{ currentVerse.confidence }}%</div>
          </div>
          
          <div class="star-display">
            <svg *ngFor="let i of [1,2,3]" 
                 width="48" height="48" 
                 viewBox="0 0 24 24" 
          ...
          <button class="confidence-continue" (click)="saveConfidence()">
            {{ currentIndex < quizVerses.length - 1 ? 'Next Verse' : 'Complete Quiz' }}
          </button>
        </div>
      </div>
      ...
      <div class="results-screen" *ngIf="showResults">
        <div class="results-card">
          <div class="results-icon" [class.passed]="passed">
            <svg *ngIf="passed" width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor">
          ...
          <div class="verse-results">
            ...
            <div class="verse-result" *ngFor="let verse of quizVerses">
              ...
              <div class="verse-score">
                <div class="score-bar">
                  <div class="score-fill" [style.width.%]="verse.confidence"></div>
                </div>
          ...
          <div class="results-message" *ngIf="!passed">
            <p>You need {{ passThreshold }}% average confidence to pass this quiz.</p>
...
export class QuizPracticeComponent implements OnInit, OnDestroy {
  ...
  async loadQuiz() {
    try {
      // Load lesson details
      const lesson = await this.workflowService.getLesson(this.lessonId, this.userId).toPromise();
      this.lesson = lesson;
      this.passThreshold = lesson?.content_data?.quiz_config?.pass_threshold || 85;
      
      // Get verses from previous lessons
      const verseCodes = await this.getQuizVerses();
      if (verseCodes.length === 0) {
        this.modalService.alert('Error', 'No verses available for quiz', 'danger');
        this.router.navigate(['/workflows', this.workflowId]);
        return;
      }
      
      // Load verse texts
      const texts = await this.bibleService.getVerseTexts(this.userId, verseCodes).toPromise();
  ...
  async getQuizVerses(): Promise<string[]> {
    ...
    const workflow = await this.workflowService.getWorkflow(this.workflowId, this.userId).toPromise();
    if (!workflow) return [];
    
    // Find current lesson position
    const currentLessonIndex = workflow.lessons.findIndex(l => l.id === this.lessonId);
    ...
    for (let i = 0; i < currentLessonIndex; i++) {
      ...
      if (lesson.content_type !== 'quiz') {
        ...
        const flashcards = await this.workflowService.getLessonFlashcards(lesson.id).toPromise();
    ...
    const randomize = this.lesson?.content_data?.quiz_config?.randomize !== false;
  ...
  getNextButtonText(): string {
    ...
    switch (this.currentVerse.currentStage) {
      case 'full': return 'Continue to FLOW';
      case 'initials': return 'Continue to Recite';
      case 'memory': return 'Rate Confidence';
      default: return 'Next';
    }
  }
  ...
  async saveConfidence() {
    ...
    await this.bibleService.saveVerse(
      this.userId,
  ...
  calculateResults() {
    const totalConfidence = this.quizVerses.reduce((sum, v) => sum + v.confidence, 0);
  ...
  updateProgress() {
    ...
    const completedStages = this.currentIndex * 3 + 
  ...
  async completeQuiz() {
    ...
    await this.workflowService.completeLesson(this.lessonId, this.userId).toPromise();
    
    // Navigate to next lesson
    const workflow = await this.workflowService.getWorkflow(this.workflowId, this.userId).toPromise();
    if (workflow?.lessons) {
      const currentIndex = workflow.lessons.findIndex(l => l.id === this.lessonId);
      const nextLesson = workflow.lessons[currentIndex + 1];
      
      if (nextLesson) {
        this.router.navigate(['/workflows', this.workflowId, 'lessons', nextLesson.id]);
      } else {
        this.router.navigate(['/workflows', this.workflowId], {
