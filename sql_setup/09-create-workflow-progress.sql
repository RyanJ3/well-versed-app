-- =====================================================
-- 09-create-workflow-progress.sql
-- Workflow enrollment and progress tracking
-- =====================================================
SET search_path TO wellversed01DEV;

-- Track which users are enrolled in which workflows
CREATE TABLE workflow_enrollments (
    ...
    current_lesson_id INTEGER REFERENCES workflow_lessons(lesson_id),
...
CREATE INDEX idx_workflow_enrollment_user ON workflow_enrollments(user_id);
CREATE INDEX idx_workflow_enrollment_workflow ON workflow_enrollments(workflow_id);

-- Track user progress for each lesson
CREATE TABLE lesson_progress (
    user_id INTEGER NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    lesson_id INTEGER NOT NULL REFERENCES workflow_lessons(lesson_id) ON DELETE CASCADE,
    workflow_id INTEGER NOT NULL REFERENCES workflows(workflow_id) ON DELETE CASCADE,
