...
first_lesson AS (
    SELECT lesson_id FROM course_lessons WHERE course_id = (SELECT course_id FROM course_cte) ORDER BY position LIMIT 1
)
INSERT INTO course_enrollments (user_id, course_id, current_lesson_id)
SELECT 1, course_id, first_lesson.lesson_id FROM course_cte, first_lesson
